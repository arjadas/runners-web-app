...
@WebMvcTest(RunController.class)  // slice test with specifically testing the RunController class
class RunControllerTest {
    ...
    @MockBean  // mocking the injection of a fake repository
    ...
    void shouldDeleteRun() throws Exception {
        var run = new Run(null, "Test", LocalDateTime.now(), LocalDateTime.now().plusMinutes(1), 1, Location.OUTDOOR, null);
        when(repository.findById(ArgumentMatchers.anyInt())).thenReturn(Optional.of(run));
